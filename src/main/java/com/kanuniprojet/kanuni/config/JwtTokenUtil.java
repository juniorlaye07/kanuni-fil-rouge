package com.kanuniprojet.kanuni.config;
...
//   compaction of the JWT to a URL-safe string
    ...
    private String doGenerateToken(Map<String, Object> claims, String subject) throws Exception {
       ...
       /* for (Role role: user.getRoles()
            ...
            RoleName junior =role.getName();
            System.out.println(junior);
            jwtauth =junior.name();
            System.out.println(jwtauth);
        }
        if (user.getStatus().equals("Bloquer")){
            throw new Exception("Votre accés est bloquer,veillez vous adressez à votre administrateur systeme!");
        }
        if (user.getPartenaire().getEtat().equals("Bloquer")){
            throw new Exception("Votre Prestataire a été bloquer!");
        }*/
        return Jwts.builder().setClaims(claims).setSubject(subject).setIssuedAt(new Date(System.currentTimeMillis())).claim("role",jwtauth)
